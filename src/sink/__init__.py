...
#    globals()['dump'] = method
    lst = ['spatial_types', 'numeric_types', 'string_types', 'date_types', 'boolean_types', 'object_types',
            'dump', 'dumps', 'loads',
...
class Schema(object):
    ...
    def add_field(self, field):
        assert (field.type in spatial_types) or \
                ...
                (field.type in date_types) or \
                (field.type in object_types), \
    ...
    def types(self):
        if self._types is None:
            self._types = [f.type for f in self.fields]
        return self._types

    @property
    def dimensions(self):
        if self._sizes is None:
            self._sizes = [f.dimension for f in self.fields]
        return self._sizes
